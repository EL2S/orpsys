document.addEventListener('DOMContentLoaded', function () {
    ...
    let allRows = Array.from(document.querySelectorAll("#leaveTable tbody tr"));
    let rows = allRows.filter(row => !row.textContent.includes("Aucun congé trouvé."));
    ...
    function renderTable() {
        ...
        // Si 0 résultat
        if (totalRows === 0) {
            // afficher la ligne "Aucun congé trouvé."
            const noDataRow = allRows.find(row => row.textContent.includes("Aucun congé trouvé."));
